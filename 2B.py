...
# This code is adapted from Dr. Jason Brownlee:
...
#    This creates the neural net with five layer and 2 neurons each

    # 1 layer
    input = X
    hidden = tf.layers.dense(inputs=input, units=2)  
    output = tf.layers.dense(inputs=hidden, units=1)
    
    # 2 layers
    hidden = tf.layers.dense(inputs=output, units=2)  
    output = tf.layers.dense(inputs=hidden, units=1)
...
#    The pred node completes the regression by multiplying the weights by the 
#    features and then adding the bias
...
#    This is where the error is calculated. It is run through the softmax cross
#    entropy function which takes in the predictions as well as the actual data
...
#    become 1, Falses become 0. The total is summed. 
    ...
    # keep track of summary for plot
...
#        the variables
        ...
        for i in range(num_Iterations):
            ...
            for b in range(0, train_X.shape[0], batch_size):
                xBatch = train_X[b:b + batch_size]
                yBatch = train_y[b:b + batch_size]

                feed_test = {X: test_X, Y: test_y}
                
                _, lossIteration = sess.run([optimizer, loss], {X: xBatch, Y: yBatch})
            final_accuracy = sess.run(accuracy, {X: test_X, Y:test_y})
            
            # so the values aren't printed with every iteration
            if i % display_step == 0:
                currentLoss, train_accuracy = sess.run([loss, accuracy], {X: xBatch, Y: yBatch})
                print('Iteration: ', i,' Loss: ', lossIteration, ' Accuracy: ',final_accuracy/test_X.shape[0])
                print('Total time: {0} seconds'.format(time.time() - start_time))

                _, c, currentAcc = sess.run([optimizer, loss, accuracySummary], feed_dict=feed_test)
